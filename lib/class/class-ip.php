...
class IP
{
    function ipCity( $userip, $chart = 'UTF-8' )
    {
        //IP数据库路径，这里用的是QQ IP数据库 20110405 纯真版
        $dat_path = dirname(__FILE__).'/QQ/qqwry.dat';
        //判断IP地址是否有效
        ...
        if($chart=='utf-8'){
            return iconv("GBK", "UTF-8", $useripaddr);
        }elseif($chart=='gbk'){
            return iconv("UTF-8", "GBK", $useripaddr);
        }elseif($chart=='gb2312'){
            return iconv("UTF-8", "GB2312", $useripaddr);
        }else{
            return $useripaddr;
        }
    }
