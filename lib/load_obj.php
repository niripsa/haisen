...
class Import
{
    ...
    // 操作excel对象
    public function excel( $encode = "UTF-8" )
    {
        $_this = Import::instance();
    ...
    public function & session( $dir = null, $stag = 1, $session_id = null )
    {
        $_this = Import::instance();
        if ( ! class_exists( 'Database' ) )
        {
    ...
    // hay 创建的缓存文件
    public function & cache( $dir = null, $parms = array() )
    {
        $_this = Import::instance();
    ...
    public function ecshop_smtp( $data = array() )
    {
        $_this = Import::instance();
    ...
    public function & xmlAmazon($parms)
    {
        $_this = Import::instance();
    ...
    public function & pool( $dir = null, $filename = 'pool' )
    {
        $_this = Import::instance();
     ...
     *  引入微信JSSDK
     */
    public function wx_jssdk( $appId, $appSecret )
    {
        $_this = Import::instance();
        if ( ! class_exists( 'JSSDK' ) )
        {
            require_once( $_this->classDir . DS . 'jssdk.php' );
        }
        return new JSSDK( $appId, $appSecret );
    }
}