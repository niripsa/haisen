<?php
class Crowd_shoppingController extends Controller{
    /*
     * @Photo Index
     * @param <type> $page
     * @param <type> $type
    ...
    function pay2() {
        ...
        $sql = "SELECT * FROM `{$this->App->prefix()}crowd_goods_order_info`";
        $sql .=" WHERE order_id = '$oid'";    
        $rt['orderinfo'] = $this->App->findrow($sql);
        if ( empty( $rt['orderinfo'] ) ) { $this->jump(ADMIN_URL);exit; }
        $sql = "SELECT * FROM `{$this->App->prefix()}crowd_goods_order` WHERE order_id = '$oid'";
        $rt['goodslist'] = $this->App->find($sql);
        
        $this->set('rt',$rt);
        $mb = $GLOBALS['LANG']['mubanid'] > 0 ? $GLOBALS['LANG']['mubanid'] : '';
        $this->template($mb.'/crowd_shopping_order_pay');
    }
    ...
    function fastpay2(){
        ...
        $uid = $this->Session->read('User.uid');
        $sql = "SELECT * FROM `{$this->App->prefix()}crowd_goods_order_info` WHERE order_id='$oid' LIMIT 1";
        $rt  = $this->App->findrow($sql);
...
/*          $appid = $this->Session->read('User.appid');
    ...
    function pay_successs_tatus2($rt=array()){
        ...
        if ( $status == '1' && ! empty($order_sn) ) {       
            ...
            $dd['pay_status']   = '1';
            $dd['pay_time']     = time();
            $this->App->update('crowd_goods_order_info',$dd,'order_sn',$order_sn);
   ...
   function update_user_tree($puid = 0,$ppuid=0)
    ...
    function get_openid_AND_pay_info(){
        ...
        $sql = "SELECT order_sn,order_amount,pay_status,shipping_fee FROM `{$this->App->prefix()}crowd_goods_order_info` WHERE pay_status = '0' AND order_sn='$order_sn' LIMIT 1";
        $rt = $this->App->findrow($sql);
        ...
        if($rt['pay_status']=='1'){
            $this->jump(str_replace('/WxPay','',ADMIN_URL).'user.php?act=crowd_orderlist');exit;
        }
        $rt['openid'] = $wecha_id;
    ...
    function _alipayment( $rt=array() ) {
        ...
        if($pay_id=='4'){ //微信支付
            $this->jump(ADMIN_URL.'WxPay/crowd_jsapi.php?order_sn='.$order_sn);exit;    
        }
        if($pay_id=='6'){ //云支付
        ...
        if($pay_id=='3'){ //支付宝
            //WAP
            $paypal_form = "<form name='aqua' method='post' action='".ADMIN_URL."paywx/crowd_alipayapi.php'>
...
/*      $fn = SYS_PATH.'data/goods_spend_gift.php';
    ...
    function checkout(){
        ...
        $goods_con = "group_id = {$goods_id}";
        $sql = "SELECT * FROM `{$this->App->prefix()}goods_crowdbuy` WHERE {$goods_con} LIMIT 1";
        ...
        $this->set('rt',$rt);
        $mb = $GLOBALS['LANG']['mubanid'] > 0 ? $GLOBALS['LANG']['mubanid'] : '';
        $this->template($mb.'/crowd_mycart_checkout');
    }
    ...
    function confirm() {
        ...
        if ( isset($_POST) && !empty($_POST) ) {
            ...
            foreach ( $cartlist as $k => $row ) {
                ...
                $mprice = $onetotal = $mprices = $total = $row['price'];
            ...
            $orderdata['order_amount'] = format_price($total); //优惠后的价格,也就是最终支付价格
            $orderdata['add_time']     = time();
            $orderdata['shipping_fee'] = 0; //邮费
            
            if ( $this->App->insert('crowd_goods_order_info',$orderdata) ) { //订单成功后
                $iid = $this->App->iid();                
                foreach ( $data as $kk => $rows ) {
                    $rows['order_id'] = $iid;                    
                    $this->App->insert('crowd_goods_order',$rows);  //添加订单商品表
                }
                                
                $this->jump(ADMIN_URL.'crowd_mycart.php?type=pay2&oid='.$iid);exit;
