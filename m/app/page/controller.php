...
class PageController extends Controller {
    ...
    public function get_site_nav( $t = 'top', $list = 4 ) 
    {
    ...
    public function index()
    {
        $this->action( 'common', 'checkjump' );
        $this->js( array( 'jquery.json-1.3.js', 'goods.js?v=17' ) ); // 将js文件放到页面头
        $this->css( array( 'flexslider.css' ) );            // 轮播js css
        $this->js( array( 'jquery.flexslider-min.js', 'main.js' ) );
        ...
        /* 专属产品 名称 价格 图片 */
        $field = 'goods_id, goods_name, pifa_price, goods_thumb';
        $sql = "SELECT {$field} FROM `{$this->App->prefix()}goods` WHERE goods_zone = '1' AND is_on_sale = '1' AND store_id = '0' ORDER BY sort_order LIMIT 0, 5";
        $goods_info['distribution'] =  $this->App->find( $sql );

        /* 我的团购 */
        $field = 'group_id, group_name, price, goods_img, number';
        $sql = "SELECT {$field} FROM `{$this->App->prefix()}goods_groupbuy` WHERE active = '1' ORDER BY sort LIMIT 0, 4";
        $goods_info['groupbuy'] =  $this->App->find( $sql );

        /* 会员专区 */
        $field = 'goods_id, goods_name, pifa_price, goods_thumb';
        $sql = "SELECT {$field} FROM `{$this->App->prefix()}goods` WHERE goods_zone = '2' AND is_on_sale = '1' AND store_id = '0' ORDER BY sort_order LIMIT 0, 4";
        $goods_info['member_dis'] =  $this->App->find( $sql );

        /* 提前出海 */
        $field = 'group_id, group_name, price, goods_img';
        $sql = "SELECT {$field} FROM `{$this->App->prefix()}goods_crowdbuy` WHERE active = '1' ORDER BY group_id DESC LIMIT 0, 3";
        $goods_info['crowdbuy'] =  $this->App->find( $sql );

        /*商铺分类*/
        $field = '*';
        $sql = "SELECT {$field} FROM `{$this->App->prefix()}store_class` ORDER BY class_sort DESC";
        $goods_info['store_class'] =  $this->App->find( $sql );
        ...
        $this->set( 'mubanid', $GLOBALS['LANG']['mubanid'] );
        $mb = $GLOBALS['LANG']['mubanid'] > 0 ? $GLOBALS['LANG']['mubanid'] : '';
        $this->template( $mb . '/page_index_201608' );
    }
