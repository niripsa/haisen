	...
	function confirm(){
		...
		if(isset($_POST)&&!empty($_POST)){
			...
			if($jifen_onetotal > $mypoints){ 
				$this->jump(ADMIN_URL.'excart.php?type=checkout',0,'你的积分不足无法兑换该商品!'); exit;
			}
			...
			if($this->App->insert('goods_order_info',$orderdata)){ //订单成功后
				...
				if($this->App->insert('user_point_change',array('time'=>mktime(),'changedesc'=>"用积分兑换商品",'uid'=>$uid,'points'=>-intval($jifen_onetotal)))){
					...
					$jifen_onetotal = intval(-$jifen_onetotal) * $pointnum;
				...
				$this->jump(ADMIN_URL.'excart.php',0,'你的订单没有提交成功，我们会尽快处理出现问题！'); exit;
			}
		...
		$this->App->write('excart',null);
		$this->jump(ADMIN_URL,0,'意外错误，我们会尽快处理出现问题！'); exit;
	}
