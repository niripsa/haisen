<?php
class Crowd_catalogModel extends Model{
		//获取分类下的商品数量[商品分类页面]
		function __get_goods_count_category($data=array()){
			  $w = $this->__category_goods_where($data);
			  $sql = "SELECT COUNT(distinct g.goods_id) FROM `{$this->prefix()}goods_crowdbuy` AS g";
		...
		function __get_categoods_list_category($data,$orderby,$start,$list){
			$w = $this->__category_goods_where($data);
			$sql = "SELECT * FROM `{$this->prefix()}goods_crowdbuy` LIMIT $start, $list";
			$rt = $this->find($sql);
			$goodslist = array();
			if(!empty($rt)){
				foreach($rt as $k=>$row){
