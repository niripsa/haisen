<link type="text/css" rel="stylesheet" href="<?php echo ADMIN_URL;?>tpl/11/css.css" media="all" />
...
<div style="height:30px; padding-top:10px; padding-left:10px; padding-right:10px; background:#015a9e; border-bottom:1px solid #ccc; color:#fff">
...
<div id="main" style="padding-top:0px; min-height:300px">
	...
	<form action="<?php echo ADMIN_URL;?>mycart.php?type=confirm" method="post" name="CONSIGNEE_ADDRESS" id="CONSIGNEE_ADDRESS">
		<input type="hidden" value="false" id="is_null" name="is_null">
		...
		<?php $userress_id = 0; foreach($rt['userress'] as $row){?>
		  ...
		  <a href="javascript:;" onclick="ressinfoop('<?php echo $row['address_id'];?>','showupdate',this)" style="border-radius:5px;display:block;background:#015a9e;cursor:pointer;width:60px; height:22px; line-height:22px; font-size:12px; color:#FFF; text-align:center">修改</a>
		  </p>
		  </td>
		  </tr>
		  <?php } }?>
		   ...
		   ?>
			...
			<tr>
				...
				<p style="padding-left:10px; padding-right:36px; position:relative; line-height:18px;">
					...
					<?php if(!empty($row['buy_more_best'])){echo '<br />该商品实行<font style="color:#FE0000;font-weight:bold">['.$row['buy_more_best'].']</font>促销活动，欢迎订购！';}?><span style="padding:2px 5px 2px 5px; color:#015a9e; cursor:pointer; position:absolute; right:5px; top:0px; z-index:22; background:#fafafa; border:1px solid #ededed;border-radius:5px;" class="delcartid" id="<?php echo $k;?>">删除</span></p>
				<?php if(!empty($row['spec'])){
				 ...
				 <p style=" padding-left:10px;font-size:12px;line-height:20px;" class="raturnprice raturnprice<?php echo $k;?>">零售价:<font color="#5f5f5f">￥<?php echo $row['shop_price'];?></font>&nbsp;&nbsp;本店价:<font color="#015a9e" class="gprice<?php echo $k;?>">￥<?php echo $row['price']>0 ? $row['price']  : $row['pifa_price'];?></font></p>
				 <div class="item" style="height:20px; line-height:20px; position:relative; padding-left:10px; padding-top:7px">
						...
						&nbsp;&nbsp;小计:<font color="#015a9e" class="gzprice<?php echo $k;?>">￥<?php echo $row['price']*$row['number'];?></font>
				  </div>
				</td>
			</tr>
...
<!--			<p style="height:20px; line-height:20px;">
			...
			<p style="line-height:40px; height:40px; background:#5594C5; text-align:center; font-size:14px; color:#fff; padding-top:5px;">实付金额:￥<span class="ztotals"><?php echo $zp;?></span>+￥<span class="freeshopp"><?php echo $free[0];?></span>(邮费)=<span class="freeshoppandprice"><?php echo ($zp+$free[0]);?></span>元</p>
			
			<p style="height:30px; line-height:30px; margin-top:10px;">
			<input value="提交订单" type="submit" align="absmiddle" onclick="return checkvar()" style="width:100%; height:40px; line-height:40px; background:#015a9e; font-size:18px; color:#FFFFFF; font-weight:bold; text-align:center; cursor:pointer;"/>
			</p>
	  </div> 
	</form>
	</div>
...
<?php $this->element('11/footer',array('lang'=>$lang)); ?>