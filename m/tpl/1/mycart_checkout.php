...
<div style="height:30px; padding-top:10px; padding-left:10px; padding-right:10px; background:#f22f6b; border-bottom:1px solid #ccc; color:#fff">
...
<div id="main" style="padding-top:0px; min-height:300px">
	...
	<form action="<?php echo ADMIN_URL;?>mycart.php?type=confirm" method="post" name="CONSIGNEE_ADDRESS" id="CONSIGNEE_ADDRESS">
		...
		<?php $userress_id = 0; foreach($rt['userress'] as $row){?>
		  ...
		  <a href="javascript:;" onclick="ressinfoop('<?php echo $row['address_id'];?>','showupdate',this)" style="border-radius:5px;display:block;background:#f22f6b;cursor:pointer;width:60px; height:22px; line-height:22px; font-size:12px; color:#FFF; text-align:center">修改</a>
		  </p>
		  </td>
		  </tr>
		  <?php } }?>
		   ...
		   ?>
			...
			<tr>
				...
				<?php if(!empty($row['spec'])){
				 ...
				 <p style=" padding-left:10px;font-size:12px;line-height:20px;" class="raturnprice raturnprice<?php echo $k;?>">零售价:<font color="#5f5f5f">￥<?php echo $row['shop_price'];?></font>&nbsp;&nbsp;本店价:<font color="#FF0000" class="gprice<?php echo $k;?>">￥<?php echo $row['price']>0 ? $row['price']  : $row['pifa_price'];?></font></p>
		...
		<div style="padding-left:10px; padding-right:10px;">
			<p style="padding-bottom:10px; padding-top:10px;"><a href="javascript:;" onclick="return ajax_clear()" style="cursor:pointer; background:url(<?php echo ADMIN_URL.'tpl/1/images/iconfont-delete.png';?>) left center no-repeat; padding-left:28px; height:30px; line-height:30px; display:block">清空购物车</a></p>
...
<!--			<p style="height:20px; line-height:20px;">
			...
			<p style="line-height:40px; height:40px; background:#FEE4CD; text-align:center; font-size:14px; color:#9A0000; padding-top:5px;">实付金额:￥<span class="ztotals"><?php echo $zp;?></span>+￥<span class="freeshopp"><?php echo $free[0];?></span>(邮费)=<span class="freeshoppandprice"><?php echo ($zp+$free[0]);?></span>元</p>
			
			<p style="height:30px; line-height:30px; margin-top:10px;">
			<input value="提交订单" type="submit" align="absmiddle" onclick="return checkvar()" style="width:100%; height:40px; line-height:40px; background:#f22f6b; font-size:18px; color:#FFFFFF; font-weight:bold; text-align:center; cursor:pointer;"/>
			</p>
	  </div> 
	</form>
	</div>
