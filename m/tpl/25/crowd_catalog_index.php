...
<?php $imgs = array(); if(!empty($rt['categoodslist']))foreach($rt['categoodslist'] as $k=>$row){ $imgs[] = $row['goods_img'];?>
	<li style="width:50%; float:left; position:relative">
		...
		<a style="background:#fff; padding:5px; display:block;" href="<?php echo ADMIN_URL.'crowd_mycart.php?type=checkout&id='.$row['group_id'];?>">
			...
			<p style="line-height:45px; height:45px; overflow:hidden;">
                ...
                ￥<?php echo str_replace('.00','',$row['price']);?>
                </b>
            </p>
		</a>
		</div>
		<a href="<?php echo ADMIN_URL.'crowd_mycart.php?type=checkout&id='.$row['group_id'];?>"><span class="buyfals">立即购买</span></a>
	</li>
<?php } ?>
<div class="clear"></div>
</ul>
