...
<!--<div style="background:#104e0d; line-height:44px; padding-left:10px; padding-right:10px;">
    <div style="width:80%; float:left;height:44px; text-align:right">
      <input placeholder="输入产品名称" type="text" name="key" style="background:#FFF; height:30px;border-radius:8px; border:1px solid #ededed; width:100%; margin-bottom:3px; line-height:normal; color:#999; text-indent:2em" />
    </div>
...
<div id="main" style="padding-top:0px; min-height:300px">
    ...
    <form action="<?php echo ADMIN_URL;?>mycart.php?type=confirm" method="post" name="CONSIGNEE_ADDRESS" id="CONSIGNEE_ADDRESS">
         ...
         <?php 
            if ( ! empty( $goodslist ) ) {
                ...
                foreach ( $goodslist as $k => $row ) {
                    ...
                    // 赠品去掉
                    if ( $row['is_alone_sale'] == '0' && (empty( $rt['gift_goods_ids'] ) || !in_array( $row['goods_id'], $rt['gift_goods_ids'] ) ) )
                    { //条件不满足者  不允许购买赠品
                        $gid = $row['goods_id'];
        ...
        ?>
            ...
            <tr>
                ...
                <p style="padding-left:10px; padding-right:36px; position:relative; line-height:18px;">
                    ...
                    <?php if(!empty($row['buy_more_best'])){echo '<br />该商品实行<font style="color:#FE0000;font-weight:bold">['.$row['buy_more_best'].']</font>促销活动，欢迎订购！';}?><span style="padding:2px 5px 2px 5px; color:#ee3838; cursor:pointer; position:absolute; right:5px; top:0px; z-index:22; background:#fafafa; border:1px solid #ededed;border-radius:5px;" class="delcartid" id="<?php echo $k;?>">删除</span></p>
                <?php if ( ! empty( $row['spec'] ) ) {
                 ...
                 <p style=" padding-left:10px;font-size:12px;line-height:20px;" class="raturnprice raturnprice<?php echo $k;?>">零售价:<font color="#5f5f5f">￥<?php echo $row['shop_price'];?></font>&nbsp;&nbsp;本店价:<font color="#ee3838" class="gprice<?php echo $k;?>">￥<?php echo $row['price']>0 ? $row['price']  : $row['pifa_price'];?></font></p>
                 <div class="item" style="height:20px; line-height:20px; position:relative; padding-left:10px; padding-top:7px">
                        ...
                        &nbsp;&nbsp;小计:<font color="#ee3838" class="gzprice<?php echo $k;?>">￥<?php echo $row['price'] * $row['number']; ?></font>
                  </div>
                </td>
            </tr>
            <?php } } ?>
        ...
        <table border="0" cellpadding="0" cellspacing="0" style="width:100%; margin-top:10px; border-bottom:1px solid #ededed">
            ...
            <tr>
                <td align="right" width="22%">我的积分：</td>
               <input type="hidden" name="mypoints"  value="<?php echo $rt['mypoints'];?>"/>
                <td>
                <?php echo $rt['mypoints'];?>分
                </td>
            </tr>
            <tr>
                <td align="right" width="22%">本次使用：</td>
                <td>
                <font color="#5f5f5f"><input type='text' id="use_point" name="use_point" value='' onkeyup="value=value.replace(/[^\d]/g,'');"  style="border:1px solid #ccc; width:30px;"/></font>
                额度<font color="#ee3838" class="point"><?php echo $rt['total_point'];?></font>积分
                </td>
            </tr>
        ...
        <div style="padding-left:10px; padding-right:10px;">
            <p style="padding-bottom:10px; padding-top:10px;"><a href="javascript:;" onclick="return ajax_clear()" style="cursor:pointer; background:url(<?php echo ADMIN_URL.'tpl/11/images/iconfont-delete.png';?>) left center no-repeat; padding-left:28px; height:30px; line-height:30px; display:block">清空购物车</a></p>
...
<!--            <p style="height:20px; line-height:20px;">
            ...
            <p style=" display:none;line-height:22px; color:#222; font-size:14px; font-weight:bold; color:#fff; padding-top:5px;">产品金额:￥<span class="ztotals"><?php echo $zp = $total;if($rt['discount']<100){?>&nbsp;X&nbsp;<?php echo str_replace('.00','',format_price($rt['discount']/10));?>折&nbsp;=&nbsp;<font class="ppzprice"><?php echo $zp = format_price($total*($rt['discount']/100));} ?></font></span>元</p>
            <input type="hidden" name="limit_point" value="<?php echo $rt['total_point'];?>" id="limit_point"/>
            <input type="hidden" name="store_num" value="0" id="store_num"/>
            ...
            实付金额:
                    ...
                    ￥<span class="freeshopp"><?php echo $free[0];?></span>(邮费)                  
                    - <span class="freepoint">0</span>(积分)
                    =<span class="freeshoppandprice"><?php echo ($zp+$free[0]);?></span>元</p>
                    <!--=<span class="freeshoppandprice"><?php echo ($zp+$free[0]);?></span>元</p>-->
            <p style="height:30px; line-height:30px; margin-top:10px;">
            <input type="hidden" name="store_totalmoney" value="<?php echo ($zp+$free[0]);?>" id="store_totalmoney"/>
...
$('.jia').live('click',function(){
    ...
    nn = $('.mycarts').html();
    $('.mycarts').html(parseInt(nn)+1);
    change_number(numobj,2);
});
$("#use_point").change(function(){
    var point_num;
    point_num = $("#use_point").val()? $("#use_point").val():0;
    if( <?php echo $rt['mypoints'];?> < point_num && point_num < Number($(".point").html()) ){
        alert('积分不足');
        return false;
        // $("#use_point").val( <?php echo $rt['mypoints'];?> );
        // $('.freepoint').html( <?php echo $rt['mypoints'];?> );
        // $('.freeshoppandprice').html( Number($('#store_totalmoney').val()) - <?php echo $rt['mypoints'];?> );
    }else if( Number($(".point").html()) < point_num && point_num < <?php echo $rt['mypoints'];?> ){
        alert('超过积分额度');
        return false;
        // $("#use_point").val( $(".point").html() );
        // $('.freepoint').html( $(".point").html()  );
        // $('.freeshoppandprice').html( Number($('#store_totalmoney').val()) - $(".point").html() );
    }else if( Number($(".point").html()) < point_num && <?php echo $rt['mypoints'];?> < point_num ){
        if( Number($(".point").html()) < <?php echo $rt['mypoints'];?> ){
            alert('超过积分额度');
            return false;
            // $("#use_point").val( $(".point").html() );
            // $('.freepoint').html( $(".point").html()  );
        ...
        $('.freepoint').html( point_num );
        $('.freeshoppandprice').html( Number($('#store_totalmoney').val()) - point_num * 5 );
    }
});
//改变商品价格
function change_number(obj, style){//style=1是减，2是加
    //地址ID
    ...
    id = $(obj).attr('id');   
    numbers = $(obj).val();
    //$('#store_num').attr( 'value', numbers );
    if(!(numbers>0)){
        numbers = 1;
        $(obj).val('1');
    }
    createwindow();
    $.post(SITE_URL+'mycart.php',{action:'ajax_change_price',id:id,style:style,number:numbers,shipping_id:shippingid,userress_id:userressid},function(data){ 
        removewindow();
        if(data.error=='0'){
            ...
            if( data.style == 1 ){//减
                $('.point').html( Number( $('#limit_point').val() ) - toDecimal(data.point) );
            }else{
                $('.point').html( Number( $('#limit_point').val() ) + toDecimal(data.point) );
            }
            $('#limit_point').attr( 'value', $('.point').html() );
            $('.gprice'+id).html('￥'+data.thisprice);
            $('.gzprice'+id).html('￥'+toDecimal(data.thisprice * numbers));
            ff = data.freemoney;
            $('.freeshopp').html(ff);
            $('.freeshoppandprice').html(toDecimal(toDecimal(data.prices)+toDecimal(ff)-$("#use_point").val()));
            $('#store_totalmoney').attr('value', toDecimal(toDecimal(data.prices)+toDecimal(ff)));
        }else{
            alert(data.message);
        }
    }, "json");
