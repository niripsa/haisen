...
if ( ! empty( $lang['dailiinfo'] ) )
{
    ...
    {
        $GLOBALS['LANG']['site_title'] = $lang['dailiinfo']['sitename'];
    }
...
// 网站的公共链接交换器
function get_url( $name="", $id=0, $url="", $type="", $urlobj = array() )
{   
    // $name:@链接名称 $id:链接ID $url:指定的URL $type： @类型，用户标记为分类 文章或者菜单导航等等的
...
function dump( $data )
{
    if ( $_COOKIE['WWWQDYDLUSER']['UID'] == '93' )
    {
        // echo '<pre>';
        // var_dump( $data );
        // echo '<pre>';
    ...
    if ( true )
    {
        file_put_contents( 'debug.txt', var_export( $data, true ) . PHP_EOL, FILE_APPEND );
    }
}