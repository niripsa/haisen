...
class CuxiaoController extends Controller{
 	...
 	function  __construct() {
		...
		$this->css(array('jquery_dialog.css','cuxiao.css'));
	}
	...
	function index($cid=0,$page=1,$rs= array()){
		$rt = $this->Cache->read(3600);
	 	if(is_null($rt)) {
			...
			$list = intval($list)>0 ?  intval($list) : 40 ; //每页显示多少个
			$start = ($page-1)*$list;
	...
	function get_goods_cate_tree($cid = 0)
	{
		...
		if ($this->App->findvar($sql) || $cid == 0)
		{
			...
			foreach ($res as $row)
			{
			   ...
			   $three_arr[$row['cat_id']]['url'] = get_url($row['cat_name'],$row['cat_id'],"cuxiao.php?cid=".$row["cat_id"],'goodscate',array('catalog','index',$row['cat_id']));
