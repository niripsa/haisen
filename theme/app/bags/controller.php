...
class BagsController extends Controller{
	...
	function banner2($cid=0){
		$this->layout('kong');
		$this->set('cid',$cid);
		$this->template('banner2');
	}
	
	function _getads($cid=0){
		$sql = "SELECT * FROM `{$this->App->prefix()}top_cate` WHERE tcid='$cid'";
		return $this->App->findrow($sql);
	}
	
	function bags(){
		$this->css(array("indexbags.css","bags.css"));
		$this->js(array("bags.js","pptBox.js"));
		$cid = 477;
		$rt = $this->Cache->read(3600);
	 	if(is_null($rt)) {
			...
			$sql = "SELECT tb1.*,tb2.goods_name,tb2.goods_thumb,tb2.pifa_price,tb2.shop_price FROM `{$this->App->prefix()}top_cate_goods` AS tb1 LEFT JOIN  `{$this->App->prefix()}goods` AS tb2 ON tb1.goods_id=tb2.goods_id WHERE tcid='15' LIMIT 6";
			...
			$sql = "SELECT tb1.*,tb2.goods_name,tb2.goods_thumb,tb2.pifa_price,tb2.shop_price,tb2.cat_id FROM `{$this->App->prefix()}top_cate_goods` AS tb1 LEFT JOIN  `{$this->App->prefix()}goods` AS tb2 ON tb1.goods_id=tb2.goods_id WHERE tcid='14' LIMIT 4";
			$rt['topgoods'] = $this->App->find($sql);
				
			$sql = "SELECT * FROM `{$this->App->prefix()}top_cate` WHERE tcid='16'";
			$rt['cuxiaocat'] = $this->App->findrow($sql);
				
			$sql = "SELECT tb1.*,tb2.goods_name,tb2.goods_thumb,tb2.pifa_price,tb2.shop_price,tb2.cat_id FROM `{$this->App->prefix()}top_cate_goods` AS tb1 LEFT JOIN  `{$this->App->prefix()}goods` AS tb2 ON tb1.goods_id=tb2.goods_id WHERE tcid='16' LIMIT 6";
			$rt['cuxiao'] = $this->App->find($sql);
			
			$sql = "SELECT * FROM `{$this->App->prefix()}top_cate` WHERE tcid='17'";
			$rt['newpincat'] = $this->App->findrow($sql);
			$sql = "SELECT tb1.*,tb2.goods_name,tb2.goods_thumb,tb2.pifa_price,tb2.shop_price,tb2.cat_id FROM `{$this->App->prefix()}top_cate_goods` AS tb1 LEFT JOIN  `{$this->App->prefix()}goods` AS tb2 ON tb1.goods_id=tb2.goods_id WHERE tcid='17' LIMIT 6";
			$rt['newpin'] = $this->App->find($sql);
	...
	function index($cid=0,$page=0,$rs= array()){
		$rt = $this->Cache->read(3600);
	 	if(is_null($rt)) {
			...
			$cid = (isset($_GET['cid'])&&intval($_GET['cid'])>0) ? intval($_GET['cid']) : 477;
