<?php
class MeizhuangController extends Controller{
	...
	function meizhuang(){
		$this->css("indexmeizhuang.css");
		$this->js(array("pptBox.js"));
		$cid = 466;
		$rt = $this->Cache->read(3600);
	 	if(is_null($rt)) {
			...
			$sql = "SELECT * FROM `{$this->App->prefix()}top_cate` WHERE parent_id='22'";
			$rt['cats'] = $this->App->find($sql); //print_r($rt['cats']);
			if(!empty($rt['cats']))foreach($rt['cats'] as $k=>$row){
				$c = $row['tcid'];
				$sql = "SELECT tb1.*,tb2.goods_name,tb2.goods_thumb,tb2.pifa_price,tb2.shop_price,tb2.cat_id FROM `{$this->App->prefix()}top_cate_goods` AS tb1 LEFT JOIN  `{$this->App->prefix()}goods` AS tb2 ON tb1.goods_id=tb2.goods_id WHERE tcid='$c' LIMIT 4";
