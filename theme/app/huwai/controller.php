<?php
class HuwaiController extends Controller{
	...
	function huwai(){
		$this->css(array("indexhuwai.css","bags.css"));
		...
		$rt = $this->Cache->read(3600);
	 	if(is_null($rt)) {
			...
			$sql = "SELECT tb1.*,tb2.goods_name,tb2.goods_thumb,tb2.pifa_price,tb2.shop_price FROM `{$this->App->prefix()}top_cate_goods` AS tb1 LEFT JOIN  `{$this->App->prefix()}goods` AS tb2 ON tb1.goods_id=tb2.goods_id WHERE tcid='41' LIMIT 6";
			...
			$sql = "SELECT tb1.*,tb2.goods_name,tb2.goods_thumb,tb2.pifa_price,tb2.shop_price,tb2.cat_id FROM `{$this->App->prefix()}top_cate_goods` AS tb1 LEFT JOIN  `{$this->App->prefix()}goods` AS tb2 ON tb1.goods_id=tb2.goods_id WHERE tcid='40' LIMIT 4";
			$rt['topgoods'] = $this->App->find($sql);
				
			$sql = "SELECT * FROM `{$this->App->prefix()}top_cate` WHERE tcid='16'";
			$rt['cuxiaocat'] = $this->App->findrow($sql);
				
			$sql = "SELECT tb1.*,tb2.goods_name,tb2.goods_thumb,tb2.pifa_price,tb2.shop_price,tb2.cat_id FROM `{$this->App->prefix()}top_cate_goods` AS tb1 LEFT JOIN  `{$this->App->prefix()}goods` AS tb2 ON tb1.goods_id=tb2.goods_id WHERE tcid='42' LIMIT 6";
			$rt['cuxiao'] = $this->App->find($sql);
			
			$sql = "SELECT * FROM `{$this->App->prefix()}top_cate` WHERE tcid='17'";
			$rt['newpincat'] = $this->App->findrow($sql);
			$sql = "SELECT tb1.*,tb2.goods_name,tb2.goods_thumb,tb2.pifa_price,tb2.shop_price,tb2.cat_id FROM `{$this->App->prefix()}top_cate_goods` AS tb1 LEFT JOIN  `{$this->App->prefix()}goods` AS tb2 ON tb1.goods_id=tb2.goods_id WHERE tcid='43' LIMIT 6";
	...
	function index($cid=0,$page=0,$rs= array()){
		$rt = $this->Cache->read(3600);
	 	if(is_null($rt)) {
			...
			$cid = (isset($_GET['cid'])&&intval($_GET['cid'])>0) ? intval($_GET['cid']) : 465;
