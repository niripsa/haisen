    ...
    <div class="m_right" >
	 ...
	 <tr>
	  ...
	  <tr>
		...
		1.可以以图片文件名称作为商品的名称。<br />
		2.不能一次选择多张图片，只能一个个选择添加。<br />
		3.为了顺利快速上传，可以将多张图片压缩为.zip|.rar文件，然后选择上传。【体验版】<br />
		3.只接受png|gif|png后缀的文件。
		<hr />
		</td>
	  </tr>
	  <tr>
	  <td>
		<form id="uploadForm" name="uploadForm" action="<?php echo SITE_URL.'inc/';?>upload.php">
		  <table border="0" cellspacing="2" class="fu_list">
			<tbody>
			  ...
			  <tr>
				<td colspan="2" align="center" id="idMsg"><input type="button" value="开始上传" id="idBtnupload" disabled="disabled" style="padding:4px; cursor:pointer"/>
				  &nbsp;&nbsp;&nbsp;
				  <input type="button" value="全部取消" id="idBtndel" disabled="disabled" style="padding:4px; cursor:pointer"/>
				</td>
			  </tr>
			</tbody>
		  </table>
...
$("idBtnupload").onclick = function(){
	...
	$("idMsg").innerHTML = "正在添加文件到您的网盘中，请耐心等待……<br />有可能因为网络问题，出现程序长时间无响应，请点击“<a href='<?php echo SITE_URL.'suppliers.php?act=suppliers_goods_batch_add';?>'><font color='red'>取消</font></a>”重新上传文件";
...
function Finish(msg){ if(msg !=""){ alert(msg); };location.href = '<?php echo SITE_URL.'suppliers.php?act=suppliers_goods_batch_add&op=cachelist';?>'; }
