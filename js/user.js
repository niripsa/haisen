...
function submit_login_data(){
		...
		$.post(SITE_URL+'user.php',{action:'login',username:name,password:pas,vifcode:vcode,issave:issave,isauto:isauto},function(data){
			removewindow();
			if(data != ""){
				//meswindow('<br /><p>'+data+'</p>','宝泰商城登录系统提醒你',300,80);
				//JqueryDialog.Open('Stybags.com Sign-on System','<br /><p>'+data+'</p>',300,50);
				$('.login_mes').html(data);
			}else{
				$('.login_mes').html('登陆成功');
				//meswindow('<br /><p>登录成功！3秒后进入前一个页面！</p>','宝泰商城登录系统提醒你',300,80);
				JqueryDialog.Open('网站登陆系统','<br /><p style="line-height:26px">登陆成功，系统将带你进入前一个页面！</p>',300,50);
		...
		$.ajax({
		   ...
		   success: function(data){ 
				...
				if(data.error==0){
					...
					/*if(jumpurl!="" && jumpurl!=null && typeof(jumpurl)!='undefined'){
						meswindow('<br /><p>你已经注册成功！三秒后将跳转前一个页面！</p>','宝泰商城购物车系统提醒你',300,80);		
						setTimeout("jsjump('"+jumpurl+"')",3000);
					}else{
						location.href=SITE_URL+'user.php?act=regsuccess'; //注册成功
					}*/
				}else{
					JqueryDialog.Open('系统提醒你','<br /><p>'+data.message+'</p>',300,80);
				}
		   },
...
function update_user_info(types){
	   ...
	   //var fromAttr=$("#USERINFO").serialize();
		...
		$.post(SITE_URL+'user.php',{action:'ressinfoop',id:id,type:type},function(data){
			if(type=='showupdate'){																	  
				if(data !="" && typeof(data) != 'undefined'){
					...
					JqueryDialog.Open('系统提醒你',data,400,300);
				}
			}else if(type=='delete'){
		  ...
		  $.ajax({
			   ...
			   success: function(data){
					...
					if(data.error==0){
						...
						JqueryDialog.Open('系统提醒你','<br /><img src="'+SITE_URL+'theme/images/error_icon.png" align="absmiddle"/>&nbsp;&nbsp;&nbsp;'+data.message,260,60);
					}
