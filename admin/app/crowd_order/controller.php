...
class Crowd_orderController extends Controller
{
    ...
    function order_print()
    {
        ...
        //修改以打印状态
        $sql = "UPDATE `{$this->App->prefix()}crowd_goods_order_info` SET is_prints = '1' WHERE order_id IN(".implode(',',$ids_rt).")";
        $this->App->query($sql);
        
        $sql = "SELECT tb1.*,tb2.region_name AS province,tb3.region_name AS city,tb4.region_name AS district,tb5.region_name AS towns, tb6.region_name AS villages FROM `{$this->App->prefix()}crowd_goods_order_info` AS tb1";
...
//          $sql .= " LEFT JOIN `{$this->App->prefix()}user_address` AS tb7 ON tb7.user_id = tb1.user_id";
    ...
    function order_list()
    {
            ...
            $list = 12;
            $start = ($page-1)*$list;
            $sql = "SELECT COUNT(order_id) FROM `{$this->App->prefix()}crowd_goods_order_info` $w";
            ...
            $sql = "SELECT order_id, order_sn,sn_id,shipping_id,shipping_id_true, user_id, order_status, shipping_status, pay_status, consignee, add_time,is_prints, SUM( order_amount + shipping_fee ) AS tprice FROM `{$this->App->prefix()}crowd_goods_order_info` $w GROUP BY order_id $orderby LIMIT $start,$list";
    ...
    function order_info($id=0){
            ...
            $sql = "SELECT tb1.*,tb2.region_name AS province,tb3.region_name AS city,tb5.user_name,tb4.region_name AS district,tb6.region_name AS town ,tb7.region_name AS village FROM `{$this->App->prefix()}crowd_goods_order_info` AS tb1"; // look添加tb5.user_name
            $sql .=" LEFT JOIN `{$this->App->prefix()}region` AS tb2 ON tb2.region_id = tb1.province";
            $sql .=" LEFT JOIN `{$this->App->prefix()}region` AS tb3 ON tb3.region_id = tb1.city";
            $sql .=" LEFT JOIN `{$this->App->prefix()}region` AS tb4 ON tb4.region_id = tb1.district";
            ...
            $sql = "SELECT tb1.*,tb2.goods_thumb,tb2.goods_unit,tb2.goods_brief,tb2.goods_number AS storage,tb2.goods_id, IFNULL(tb3.brand_name, '') AS brand_name FROM `{$this->App->prefix()}crowd_goods_order` AS tb1";  //look 添加
            $sql .= " LEFT JOIN `{$this->App->prefix()}crowd_goods` AS tb2 ON tb1.goods_id = tb2.goods_id";
            ...
            //订单操作信息
            $sql = "SELECT * FROM `{$this->App->prefix()}crowd_goods_order_action` WHERE order_id='$id' ORDER BY log_time DESC";
            ...
            $this->set('rt',$rt);
            unset($rs);
            $this->template('crowd_goods_order_info');
    }
        ...
        */
        ...
        function ajax_op_status($data=array()){
            ...
            $order_id = $data['opid'];
            //当前购买用户
            $pu = $this->App->findrow("SELECT user_id,order_sn FROM `{$this->App->prefix()}crowd_goods_order_info` WHERE order_id = '$order_id' LIMIT 1");
            ...
            if($datas['shipping_status']=='2'){
                //查找物流单
                /*$oid = $data['opid'];
                $sql = "SELECT sn_id,shipping_id FROM `{$this->App->prefix()}crowd_goods_order_info` WHERE order_id='$oid' LIMIT 1";
                ...
                $rr3 = $this->App->findrow("SELECT * FROM `{$this->App->prefix()}crowd_goods_order_info` WHERE order_sn='{$order_sn}' LIMIT 1");
            ...
            if($datas['shipping_status'] == '5'){// 收货确认
            ...
            if($rts['userbonus']){
               // 开启收货返佣选项
                $pay_status = $this->App->findvar("SELECT pay_status FROM `{$this->App->prefix()}crowd_goods_order_info` WHERE order_id='$order_id' LIMIT 1");
              $tt = "false";
            
              if($pay_status == 1  && !empty($order_id)){
                 // 开始返佣
            ...
            $moneys = $this->App->find($sql);
                 $pu = $this->App->findrow("SELECT user_id,daili_uid,parent_uid,parent_uid2,parent_uid3,goods_amount,order_amount,order_sn,pay_status,order_id FROM `{$this->App->prefix()}crowd_goods_order_info` WHERE order_id='$order_id' LIMIT 1");
            ...
            if($parent_uid>0){
                $sql = "SELECT p1_uid,p2_uid,p3_uid,p4_uid FROM `{$this->App->prefix()}user_tuijian_fx` WHERE uid = '$parent_uid'";
                ...
                if($parent_uid==$user_id){
                    ...
                    $parent_uid4 = isset($rr['p4_uid']) ? $rr['p4_uid'] : 0; //分享者
                }else{
                    $parent_uid2 = isset($rr['p1_uid']) ? $rr['p1_uid'] : 0; //分享者
                    $parent_uid3 = isset($rr['p2_uid']) ? $rr['p2_uid'] : 0; //分享者
                    $parent_uid4 = isset($rr['p3_uid']) ? $rr['p3_uid'] : 0; //分享者
                }
            }
            ...
            $order_sn = isset($pu['order_sn']) ? $pu['order_sn'] : 0;
            ...
            if($parent_uid > 0){
                ...
                if($tjpointnum > 0 ){
                    ...
                    if($pid > 0){
                        ...
                        //$points2 = intval($scores_money * $tjpointnum * $tjpointnum_ag);
                        $points2 = 1;
...
/*                  if($types=='1'){ //全职
                ...
                if($rank=='12'){ //普通分销商
                    if( floatval($rts['team_accumulative_money']) > 0 && floatval($fTeamSum) >= floatval($rts['team_accumulative_money']) ){
                        ...
                        if(!empty($moeys)){
                            ...
                            $this->App->query($sql);
                            $this->App->insert('user_money_change',array('buyuid'=>$uid,'order_sn'=>$order_sn,'thismonth'=>$thismonth,'thism'=>$thism,'money'=>$moeys,'changedesc'=>'购买商品返佣金','time'=>mktime(),'uid'=>$parent_uid4));
            ...
            $bIsSuccess = $this->App->update('crowd_goods_order_info',$datas,'order_id',$data['opid']); //更新状态

            if($bIsSuccess && !empty($datas['shipping_time'])){
                //更新个人积累
                $field = 'user_id,order_amount';
                $id = intval($data['opid']);
                $sql = "SELECT {$field} FROM `{$this->App->prefix()}crowd_goods_order_info` WHERE order_id = '$id' LIMIT 1";
            ...
            if($datas['pay_status']=='1'){
                $order_id = $data['opid'];
                $order_sn = $this->App->findvar("SELECT order_sn FROM `{$this->App->prefix()}crowd_goods_order_info` WHERE order_id = '$order_id' LIMIT 1");
    ...
    function pay_successs_tatus2($order_sn=''){
        ...
        $pay_status = $this->App->findvar("SELECT pay_status FROM `{$this->App->prefix()}crowd_goods_order_info` WHERE order_sn='$order_sn' LIMIT 1");
        $tt = "false";
        //if($pay_status!='1'){
            //检查
        ...
        if($tt == 'true' && !empty($order_sn)){
            $pu = $this->App->findrow("SELECT user_id,daili_uid,parent_uid,parent_uid2,parent_uid3,goods_amount,order_amount,order_sn,pay_status,order_id FROM `{$this->App->prefix()}crowd_goods_order_info` WHERE order_sn='$order_sn' LIMIT 1");
            ...
            if($parent_uid3 > 0 && $userbonus == 0){
                ...
                $this->writeLog(__FILE__ . "|3|uid:{$uid}| 3 level | money:" . $moeys . "|ticheng:" . $off . '|rank:' . $rank . '|parent_id:' . $parent_uid3);
            }//end of if uid3
    ...
    function ajax_get_status_button($var=0){
        ...
        function  ajax_order_bathop($ids=0,$type=""){
            ...
            switch ($type){
                case 'bathdel':
                    //批量删除订单
                    $now_status = $this->App->findcol("SELECT order_status FROM `{$this->App->prefix()}crowd_goods_order_info` WHERE order_id IN(".implode(',',$id_arr).")");
                    if(!empty($now_status)){
                        $afterarr = array();
                    ...
                    //$sql = "DELETE FROM `{$this->App->prefix()}crowd_goods_order_info` WHERE order_id IN(".implode(',',$id_arr).")";
                    //$this->App->query($sql);
                    if(!empty($id_arr)){
                        $this->App->delete('crowd_goods_order_info','order_id',$id_arr);  //订单表
                 ...
                 case 'bathconfirm':
                    ...
                    if(!empty($id_arr)){
                        if($this->App->update('crowd_goods_order_info',array('order_status'=>'2'),'order_id',$id_arr)){
                            $sql = "SELECT tb1.user_id,tb1.order_sn,tb1.order_id,tb2.user_name,tb2.email FROM `{$this->App->prefix()}crowd_goods_order_info` AS tb1 LEFT JOIN `{$this->App->prefix()}user` AS tb2 ON tb1.user_id=tb2.user_id WHERE tb1.order_id IN(".implode(',',$id_arr).")";
                 ...
                 case 'bathcancel':
                    ...
                    if(!empty($id_arr)){
                        $this->App->update('crowd_goods_order_info',array('order_status'=>'1'),'order_id',$id_arr);
                 ...
                 case 'bathinvalid':
                    ...
                    if(!empty($id_arr)){
                        $this->App->update('crowd_goods_order_info',array('order_status'=>'4'),'order_id',$id_arr);
...
//产品总销量
    function product_order_list(){
        ...
        //  exit;
            ...
            $this->set("pagelink",$pagelink);
          /*  $sql = "SELECT order_id, order_sn, user_id, order_status, shipping_status, pay_status, consignee, add_time, SUM( goods_amount + shipping_fee ) AS tprice FROM `{$this->App->prefix()}crowd_goods_order_info` $w GROUP BY order_id $orderby LIMIT $start,$list";
            ...
            $sql .= " WHERE tb1.order_id = '$id'";
          */
          $sql = "SELECT order_id  FROM `{$this->App->prefix()}crowd_goods_order_info` $w GROUP BY order_id $orderby ";  //取得订单表id
        
        /*************************  能获取到供应商商品的订单  ****************************************
        
        $sql = "SELECT tb1.order_id , tb2.goods_id ,tb3.uid ,tb4.user_name  FROM `{$this->App->prefix()}crowd_goods_order_info` AS tb1"; 
...
/***************  look添加 结束    *************************************************************************/       
        ...
        function order_takeover(){
               ...
               $limittime = time() - $rts['takeover'] * 24 * 60 *60;
                    $sql = "Select t1.order_id from `{$this->App->prefix()}crowd_goods_order_info` as t1 Left Join  `{$this->App->prefix()}shipping_sn` as t2 on t1.sn_id = t2.shipping_sn  where  t1.shipping_status = '2' and t1.pay_status= '1' and t1.order_status = '2' and t2.addtime >= '$limittime'" ;
                    //return $sql;
                    $rr = $this->App->find($sql);
                    foreach($rr as $e){
