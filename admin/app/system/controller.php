...
class SystemController extends Controller{
    ...
    function viewshop()
    {
        ...
        if ( ! (is_file( $f )) || ! file_exists( $f ) || (time() - filemtime( $f ) > 360) )
        {
    ...
    function add_admin_log( $optionlog = "" )
    {
        ...
        if ( empty( $uname ) ) 
        {
            $import_obj->basic()->redirect('login.php',0,'非法操作：没有登陆！');
        }
        $data['optioner']  = $uname;
        $data['optiondt']  = time();
        $data['optionip']  = $import_obj->basic()->getip();
    ...
    function getcount(){
        ...
        */
        ...
        $import_obj = new Import();
        $rt['os']      = $import_obj->basic()->get_os();
        $rt['browser'] = $import_obj->basic()->get_user_browser();
        $rt['bsip']    = $import_obj->basic()->getip();
        $rt['ip_from'] = $import_obj->ip()->ipCity($rt['bsip']);
        $rt['csip']    = $import_obj->basic()->serverIP();
