...
class WeixinController extends Controller
{
    ...
    function userconfig( $data = array() )
    {
        ...
        {
            $wid = $rt['wid'];
            $sql = "SELECT nickname FROM `{$this->App->prefix()}user` WHERE user_id = '$wid' LIMIT 1";
            $rt['nickname'] = $this->App->findvar( $sql );
        }
        if ( ! empty( $_POST ) )
        {
    ...
    function ajax_u_name_shopid( $rt = array() )
    {
        $name = $rt['searchval'];
        if(empty($name)) die('');
        $sql = "SELECT nickname,user_id FROM `{$this->App->prefix()}user` WHERE (nickname LIKE '%$name%' OR mobile_phone LIKE '%$name%') LIMIT 10";
    ...
    */
    ...
    function ajax_diyclass_send($data=array()){
            ...
            foreach($class as $key=>$vo){
                ...
                $data.='{"name":"'.$vo['title'].'",';
                $id = $vo['id']; 
                $c=$this->App->find("SELECT * FROM `{$this->App->prefix()}wxdiymen` WHERE parent_id='$id' AND is_show='1' order by `sort` LIMIT 5");
    ...
    */
    function oauthinfo($data=array()){ 
        ...
        $sql = "Select * from `{$this->App->prefix()}oauth` Where `oid`={$oid}";
        $row = $this->App->findrow($sql);
         $this->set('oid',$oid);
         $this->set('rt',$row);
        $this->template('oauthinfo');
    }
    /**
     * 删除分类
     *
     */
    function delcate( $data = array() )
    {
        $ids = trim( $data['ids'] );
        if ( $ids != '' )
        {
            ...
            $sql = "DELETE FROM `{$this->App->prefix()}wx_cate` " . $where;
            ...
            {
                echo json_encode( array( 'success' => false, 'info' => 'delete error' ) );
            }
        }
    }
}
