...
<div class="contentbox">
    ...
    <?php echo ! empty( $row['sn_id'] ) ? '<a style="color:#fe0000" href="http://m.kuaidi100.com/index_all.html?type='.$row['shipping_code'].'&postid='.$row['sn_id'].'" target="_blank">'.$row['sn_id'].'</a></font>' : '<span class="vieworder" style="color:#fe0000;display:none" id="'.$row['order_id'].'">点击输入物流单</span>'; ?>
    </td>
    <td>
    ...
    <?php if ( in_array( $row['order_status'], array( '1', '4' ) ) ) { ?>
    <img src="<?php echo $this->img('icon_drop.gif');?>" title="删除" alt="删除" id="<?php echo $row['order_id'];?>" class="delorder"/>
    <?php } ?>
    </td>
    </tr>
    <?php } ?>
...
function update_2012127( id, object, sid )
{
   ...
   var data = { action:'ajax_shopping_op', oid:id, val:editval, sid:sid }
   $.post( '<?php echo ADMIN_URL.'shopping.php';?>', data, function( data ) {
        obj.html( editval );
        $(object).unbind('click');
