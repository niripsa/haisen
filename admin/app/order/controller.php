...
//          $sql .= " LEFT JOIN `{$this->App->prefix()}user_address` AS tb7 ON tb7.user_id = tb1.user_id";
    ...
    public function order_list()
    {
        ...
        $sql = "SELECT order_id, order_sn,sn_id,shipping_id,shipping_id_true, user_id, store_id, order_status, shipping_status, pay_status, consignee, add_time,is_prints, SUM( order_amount + shipping_fee ) AS tprice FROM `{$this->App->prefix()}goods_order_info` $w GROUP BY order_id $orderby LIMIT $start,$list";
        ...
        if ( ! empty( $rt ) )
        {
            foreach ( $rt as $row )
            {
                ...
                $store_info = $this->App->findrow("SELECT * FROM `{$this->App->prefix()}store` WHERE store_id = '$row[store_id]' LIMIT 1");
                $orderlist[] = array(
