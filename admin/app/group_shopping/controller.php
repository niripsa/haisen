 ...
 * 团购订单操作
 */
class Group_shoppingController extends Controller 
{
    ...
    public function ajax_shopping_op( $data ) {
        ...
        if ( $oid > 0 && $sid > 0 && ! empty( $sn ) ) {
            $row = $this->App->findrow("SELECT * FROM `{$this->App->prefix()}group_goods_order_info` WHERE order_id = '$oid' LIMIT 1");
            $ssn = $row['sn_id'];
            if ( ! empty( $ssn ) && $ssn !== $sn ) {
            ...
            $id = $this->App->findvar( $sql );
            if ( $id > 0 ) {
                $this->App->update( 'shipping_sn', array('shipping_id'=>$sid,'usetime'=>time(),'is_use'=>'1'), 'shipping_sn', $sn );
            } else {
                $this->App->insert( 'shipping_sn', array('shipping_sn'=>$sn,'shipping_id'=>$sid,'usetime'=>time(),'addtime'=>time(),'is_use'=>'1') );
            }
            if ( $this->App->update( 'group_goods_order_info', array('sn_id'=>$sn,'shipping_id_true'=>$sid), 'order_id', $oid ) ) {              
                $shopping_id = $this->App->findvar("SELECT shipping_id FROM `{$this->App->prefix()}shipping_sn` WHERE shipping_sn='{$sn}' LIMIT 1");
                $shopping_name = $this->App->findvar("SELECT shipping_name FROM `{$this->App->prefix()}shipping_name` WHERE shipping_id='{$shopping_id}' LIMIT 1");
                $this->action('sms','sms_yssend',array('tel'=>$row['mobile'],'express'=>$shopping_name,'number'=>$sn,'type'=>'tmp_goods'));
            }
