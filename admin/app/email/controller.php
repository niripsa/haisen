   ...
   //邮箱服务器配置
	function email_config(){
		...
		if(!empty($_POST)){
			...
			/*if(!empty($_POST)){
			$this->App->update('systemconfig',array('email_config'=>serialize($_POST)),'type','basic'); //序列化后保存 
			echo '<script> alert("保存成功！"); </script>';
			}
			$sql = "SELECT * FROM `{$this->App->prefix()}systemconfig` LIMIT 1";
			$rt = $this->App->findrow($sql);
			if(!empty($rt['email_config'])){
				$this->set('rt',unserialize($rt['email_config']));
			}
		...
		//echo '2';
		//require_once(SITE_URL.'data/email_config.php');
		//echo $GLOBALS['LANG']['smtp_mail'];
		//print_r($GLOBALS['LANG']['email_open_config']);exit;
