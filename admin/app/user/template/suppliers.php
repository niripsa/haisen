...
<div class="contentbox">
     ...
     <tr>
        <th colspan="9" align="left" style="text-align:left">分销用户列表</th>
    </tr>
    ...
    <td><a href="javascript:;" style=" text-align:center;position:relative; display:block; background-color:#E2E8EB; border-bottom:2px solid #ccc; border-right:2px solid #ccc; padding-left:5px"><?php echo !empty($row['nickname']) ? $row['nickname'] : '未关注'; ?></a>
    </td>
    ...
    <?php $import_obj = new Import(); ?>
    <td><?php echo !empty($row['last_login']) ? date('Y-m-d H:i:s',$row['last_login']).'<br /><font color="#FF0000">['.$import_obj->ip()->ipCity($row['last_ip']).']</font>' : '无知';?></td>
    <td><?php echo $row['reg_ip'];?><br /><font color="#FF0000">[<?php echo $row['reg_from'] ? $row['reg_from'] : '无知';?>]</font></td>
    <td>
    <a href="user.php?type=dailiapplyinfo&id=<?php echo $row['user_id'];?>&goto=suppliers" title="编辑"><img src="<?php echo $this->img('icon_edit.gif');?>" title="编辑"/></a>&nbsp;
