...
<div class="contentbox">
    ...
    <td>
        <a href="javascript:;" style="text-align:center;position:relative; display:block; background-color:#E2E8EB; border-bottom:2px solid #ccc; border-right:2px solid #ccc; padding-left:5px">
        <font color="blue">[<?php if(!empty($row['pname'])){ echo $row['pname']; } else { echo '无'; } ?>]</font>
        <?php echo !empty($row['nickname']) ? $row['nickname'] : '未关注'; ?>
        </a>
    </td>
    <td>
        <img src="<?php echo $this->img( $row['nickname'] ? 'yes.gif' : 'no.gif' ); ?>" alt="<?php echo $row['is_subscribe']==1 ? '0' : '1';?>" class="activeop" lang="active" id="<?php echo $row['id'];?>"/>
    </td>
