...
<div class="contentbox">
     ...
     <tr>
        <th colspan="6" align="left" style="text-align:left">资金统计</th>
    </tr>
    <tr>
        <td colspan="9" align="left">
        <img src="<?php echo $this->img( 'icon_search.gif' ); ?>" alt="SEARCH" width="26" border="0" height="22" align="absmiddle">
        昵称 <input name="nickname" size="15" type="text" value="<?php echo isset($_GET['nickname']) ? $_GET['nickname'] : "";?>">
        <input value=" 搜索 " class="cate_search" type="button">
        </td>
    </tr>
    <tr>
        <th width="50">
            <label><input type="checkbox" class="quxuanall" value="checkbox" />编号</label>
        </th>
       <th>昵称</th>
       <th>收入</th>
       <th>原因</th>
    ...
    <td> <?php echo $row['nickname'] ? : '未关注'; ?> </td>
    <td>
        <?php echo $row['money'] > 0 
                ? '收入:<font color=blue>￥' . $row['money'] . '</font>' 
                : '支出:<font color=red>￥' . (-$row['money']) . '</font>';
        ?>
    </td>
    <td> <?php echo $row['changedesc'];?> </td>
    <td> <?php echo date( 'Y-m-d H:i:s', $row['time'] ); ?> </td>
    <td>
        <a href="<?php echo ADMIN_URL . 'user.php?type=usermoney&id=' . $row['cid']; ?>" onclick="return confirm( '确定删除吗' );">
        <img src="<?php echo $this->img('icon_drop.gif');?>" title="删除" alt="删除" id="<?php echo $row['cid'];?>"/>
        </a>
    </td>
    </tr>
    <?php } } ?>
  ...
  //批量删除
   ...
   $('.deluserid').click(function(){
    ...
    $('.cate_search').click( function() {        
        keys = $('input[name="nickname"]').val();        
        location.href = '<?php echo ADMIN_URL . 'user.php?type=usermoney'; ?>&nickname=' + keys;
    });
</script>