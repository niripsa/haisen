...
class GoodsController extends Controller
{
    ...
    public function goods_list()
    {
        ...
        if ( isset( $_GET ) && ! empty( $_GET ) ) {
            ...
            {
                $comd[] = "tb1.store_id = ". $_GET['store_id'];
            }
            if ( isset( $_GET['wallet_id'] ) && $_GET['wallet_id'] )
            {
                $comd[] = "tb1.wallet_id = ". $_GET['wallet_id'];
            }
            if ( ! empty( $comd ) )
            {
        ...
        $field = "tb1.goods_id,tb4.store_name,tb1.sort_order,tb1.cat_id,tb1.goods_thumb, tb1.goods_sn, tb1.goods_name, tb1.is_on_sale,tb1.is_check, tb1.is_promote,tb1.is_qianggou,tb1.market_price, tb1.shop_price,tb1.pifa_price,tb1.promote_price,tb1.qianggou_price,tb1.qianggou_start_date,tb1.qianggou_end_date,tb1.promote_start_date,tb1.promote_end_date,tb1.add_time, tb1.is_shipping,tb1.is_best,tb1.is_new,tb1.is_hot,tb1.is_alone_sale,tb1.wallet_id,tb2.cat_name,tb3.user_name,tb3.nickname";
        $sql = "SELECT {$field} FROM `{$this->App->prefix()}goods` AS tb1";
        ...
        $sql .=" LEFT JOIN `{$this->App->prefix()}store` AS tb4 ON tb4.store_id=tb1.store_id";
    ...
    // 商品列表页面
    public function goods_list_all( $dd = array() ) {
        // 排序
        $orderby = "";
        if ( isset( $_GET['desc'] ) ) {
            $orderby = ' ORDER BY '.$_GET['desc'].' DESC';
    ...
    public function goods_info( $gid = 0 )
    {
        ...
        if ( $gid > 0 ) { // 编辑页面
            ...
            if ( isset( $_POST ) && ! empty( $_POST ) ) {
                if ( empty( $_POST['goods_name'] ) ) {
                    ...
                    if ( floatval($_POST['pifa_price']) < floatval($_POST['need_jifen']) ) {
                        echo '<script>alert("所需积分不能大于单价,请修改所需积分！");</script>';
                    }
                    // 货号
            ...
            if ( isset( $_POST ) && ! empty( $_POST ) ) {
                if ( empty ( $_POST['goods_name'] ) ) {
                    ...
                    if ( $_POST['store_id'] )
                    {
                        $store_info         = $this->App->findrow("SELECT * FROM `{$this->App->prefix()}store` WHERE store_id='$_POST[store_id]'");
                        $_POST['wallet_id'] = $store_info['wallet_id'];
                    }
        ...
        //商家
        $sql = "SELECT * FROM `{$this->App->prefix()}store` WHERE status=1";
        $this->set( 'uidlist', $this->App->find( $sql ) );

        /* 已开启的钱包 */
        $sql = "SELECT * FROM `{$this->App->prefix()}wallet` WHERE state = '1'";
        $this->set( 'wallet_list', $this->App->find( $sql ) );
    ...
    // 消费额度赠品管理
    public function goods_spend_gift()
    {
        $fn = SYS_PATH . 'data/goods_spend_gift.php';
