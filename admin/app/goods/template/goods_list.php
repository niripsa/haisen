...
<div class="contentbox">
    ...
    <tr>
        <th colspan="14" align="left"><span style="float:left">商品列表</span><a href="<?php echo ADMIN_URL.'goods.php?type=goods_info';?>" style="float:right; padding:2px 4px 2px 4px; background:#ededed; border-bottom:2px solid #ccc; border-right:2px solid #ccc">添加商品</a></th>
    </tr>
    <tr>
    <form action="" method="GET">
    <input type="hidden" name="type" value="goods_list" />
    <td colspan="14" align="left" class="label">
    ...
    <select name="store_id">
        ...
        ?>
        <option value="<?php echo $row['store_id']; ?>" <?php if(isset($_GET['store_id'])&&$_GET['store_id']==$row['store_id']){ echo 'selected="selected""'; } ?>><?php echo $row['store_name'];?></option>
        <?php } } ?>
    </select>
    <select name="wallet_id">
        <option value="">--选择钱包--</option>
        <?php foreach ( $wallet_list as $row ) { ?>
        <option value="<?php echo $row['wallet_id']; ?>" <?php echo $_GET['wallet_id'] == $row['wallet_id'] ? 'selected' : ''; ?>><?php echo $row['wallet_name'];?></option>
        <?php } ?>
    </select>
    ...
    <tr>
       ...
       <th><a href="<?php echo $price;?>">商家名称</a></th>
    ...
    <?php 
    $pr = ($row['promote_start_date']< time()&&$row['promote_end_date'] > time() && $row['is_promote']=='1') ? 1 : 0;
    $pn = ($row['qianggou_start_date']< time()&&$row['qianggou_end_date'] > time() && $row['is_qianggou']=='1') ? 1 : 0;
    ?>
