 ...
 * 众筹控制器
 */
class CrowdbuyController extends Controller {
    ...
    function index(){
       //分页
        ...
        $list = 10;
        $start = ($page-1)*$list;
        $sql = "SELECT COUNT(group_id) FROM `{$this->App->prefix()}goods_crowdbuy`";
        ...
        $sql = "SELECT tb1.*,tb2.goods_name,tb2.is_delete,tb2.is_on_sale FROM `{$this->App->prefix()}goods_crowdbuy` AS tb1 LEFT JOIN `{$this->App->prefix()}goods` AS tb2 ON tb1.goods_id=tb2.goods_id ORDER BY tb1.group_id DESC LIMIT $start,$list";
        
        $this->set('rt',$this->App->find($sql));
        $this->template('crowdbuy_index');
    }
    
    /*添加及编辑众筹信息
     @$id:大于0时，为编辑页面，否则为添加
    */
    function crowdinfo( $id = 0 ) 
    {
        ...
        if ( $id > 0 ) { //编辑
            if ( ! empty( $_POST ) ) {
                ...
                $up_res = $this->App->update( 'goods_crowdbuy', $data, 'group_id', $id );
                if ( $up_res !== false ) {
                    if ( ! empty( $dd ) ) {
                        foreach ( $dd as $row ) {
                            $this->App->insert('goods_crowdbuy_price',$row);
                        }
                    }
                    unset($data,$dd);
                    $this->jump( '', 0, '更新成功！' );
            ...
            $sql = "SELECT tb1.*,tb2.goods_name,tb2.cat_id,tb2.brand_id,tb2.is_delete,tb2.is_on_sale FROM `{$this->App->prefix()}goods_crowdbuy` AS tb1 LEFT JOIN `{$this->App->prefix()}goods` AS tb2 ON tb1.goods_id = tb2.goods_id WHERE tb1.group_id='$id' LIMIT 1";
            $rt = $this->App->findrow( $sql );
            if ( empty( $rt ) ) {
                $this->jump('crowdbuy.php?type=list'); exit;
            }
            $_GET['cat_id'] = $rt['cat_id'];
            $_GET['brand_id'] = $rt['brand_id'];
            
            $sql = "SELECT tb2.* FROM `{$this->App->prefix()}goods_crowdbuy` AS tb1";
            $sql .=" LEFT JOIN `{$this->App->prefix()}goods_crowdbuy_price` AS tb2 ON tb1.group_id = tb2.group_id";
            ...
            if ( !empty( $_POST ) ) {
                ...
                $data['qingdan']    = $_POST['qingdan'];
                $data['active']     = $_POST['active'] == 1 ? $_POST['active'] : 0;
                if ( $this->App->insert( 'goods_crowdbuy', $data ) ) {
                    $iid = $this->App->iid();
                    foreach ( $dd as $row ) {
                        $row['group_id'] = $iid;
                        $this->App->insert( 'goods_crowdbuy_price', $row );
    ...
    AJAX删除团购商品价格
    */
    function ajax_del_crowd_goods($id=0){
        if(!($id>0)){ echo "传送ID非法！"; exit;}
        $this->App->delete('goods_crowdbuy_price','gpid',$id);
    }
    
    /*
    AJAX删除众筹商品
    */
    function ajax_delcrowd($ids=0){
        ...
        if(!is_array($ids))
            ...
            $id_arr = $ids; 
        if(!empty($ids)){
            $sql = "delete from `{$this->App->prefix()}goods_crowdbuy` WHERE group_id IN(".implode(',',$id_arr).")"; 
            $this->App->query($sql);
