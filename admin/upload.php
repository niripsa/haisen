...
$GLOBALS['picdir'] = SYS_PATH_PHOTOS.$yuming.(isset($_REQUEST['tyy'])&&!empty($_REQUEST['tyy'])? trim($_REQUEST['tyy']) : trim($_REQUEST['ty'])).'/'.date('Ym',time()).'/'; 
if(isset($_POST)&&!empty($_POST)){
	$name = $app->action('upload','upload','sf_upfile',$GLOBALS['picdir']);
	if(!empty($name)) $action	= "show";
	$GLOBALS['picurl'] = 'photos/'.$yuming.(isset($_REQUEST['tyy'])&&!empty($_REQUEST['tyy'])? trim($_REQUEST['tyy']) : trim($_REQUEST['ty'])).'/'.date('Ym',time()).'/'.$name;

	if($GLOBALS['picurl']){
	   switch(basename(!empty($_REQUEST['tyy'])? trim($_REQUEST['tyy']) : trim($_REQUEST['ty']))){
	     case  'qrcard':
			$c = $app->action('save','qrcard');
			break;
		 case 'logo':
			$c = $app->action('save','logo');
			break;
	   }
	}
}
