...
if ( $_POST['action'] ) {
    switch ( $_POST['action'] ) {
        ...
        case 'ajax_rp_mes':
            ...
            break;
        case 'ajax_user_bonus':
            $app->action('user','ajax_user_bonus',$_POST); 
            break;
        case 'ajax_user_bonus_action':
            $app->action('user','user_bonus_action',$_POST); 
            break;
        default:
...
if ( isset( $_GET['type'] ) ) {
    switch ( $_GET['type'] ) {
        ...
        case 'salesmen_manage':
            ...
            break;
        case 'user_bonus':
            $app->action('user','user_bonus'); 
            break;
        case 'user_bonus_list':
            $app->action('user','user_bonus_list'); 
            break;
        case 'user_bonus_detail':
            $app->action('user','user_bonus_detail',($_GET['id'] ?  $_GET['id'] : 0)); 
            break;
        default:
