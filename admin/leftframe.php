...
<style type="text/css">
.list_tilte_onclick span{line-height: 62px;
font-size: 20px;
color: #8CBA51;}
...
output += "<span class=icon></span><span>" + outlookbar.titlelist[i].title + "</span>"; 
output += "</div>"; 
output += "<div class=list_detail id=sub_detail_" + i + "><ul>"; 
for(j = 0; j < outlookbar.itemlist[i].length; j ++ ) 
{ 
output += "<li id=" + outlookbar.itemlist[i][j].sortname + j + " onclick=\"changeframe('"+outlookbar.itemlist[i][j].title+"', '"+outlookbar.titlelist[i].title+"', '"+outlookbar.itemlist[i][j].key+"')\"><a href=#>" + outlookbar.itemlist[i][j].title + "<span class=icon></span></a></li>" 
} 
output += "</ul></div>" 
} 
} 
...
if(item != "" && sortname != "") 
{ 
window.top.frames['topFrame'].getObject('show_text').innerHTML = sortname + "  <img src=images/slide.gif broder=0 />  " + item 
} 
if(src != "") 
{ 
...
<body onload="initinav('管理首页')">
	...
	<script type="text/javascript"
		src="<?php echo ADMIN_URL;?>js/jquery1.6.js"></script>
